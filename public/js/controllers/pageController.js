...
(function(app, push, check, ctrlpad){
    ...
    app.controller('PageController',
        ...
        */
        ...
        var defaultPath = ctrlpad.pageCtrl.path,
            defaultLimit = 10,
        ...
        var dRetriever = function(curPage,  limit, orderByField, sequence, nPath){
            ...
            var promise = shared.requestPromise.then(function(v){
                ...
                var tempPages = Math.ceil($scope.pages.length/limit);

                var res = [];
                var res = v.data.map(function(user, i){
                    var a = [],offset = (curPage -1) * limit + i;
                    a = $scope.pages.filter(function(n){
                        return n.id == user.id;
                    });
                    ...
                    if(!a.length){
                        ...
                        for(;i <= offset; i++){
                            $scope.pages.splice(i,0,user);
                        }    
                        
                        $scope.pages.splice(offset,1,user);    
                    }
