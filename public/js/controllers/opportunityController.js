...
(function(app, push, check, ctrlpad){
    ...
    app.controller('OpportunityController',
        ...
        * operations here
        */
        var path =  ctrlpad.opportunityCtrl.path;
        var defaultPath = ctrlpad.opportunityCtrl.path,
            defaultLimit = 10,
        ...
        var dRetriever = function(curPage,  limit, orderByField, sequence, nPath){
            ...
            var promise = shared.requestPromise.then(function(v){
                ...
                var tempPages = Math.ceil($scope.opportunities.length/limit);

                var res = [];
                var res = v.data.map(function(user, i){
                    var a = [],offset = (curPage -1) * limit + i;
                    a = $scope.opportunities.filter(function(n){
                        return n.id == user.id;
                    });
                    
                    if(!a.length){
                        var i= $scope.opportunities.length;  
                        for(;i <= offset; i++){
                            $scope.opportunities.splice(i,0,user);
                        }    
                        
                        $scope.opportunities.splice(offset,1,user);    
                    }

                    return user;    
                });
                
                //$scope.pagesData.push({'page':curPage,'data':res});
